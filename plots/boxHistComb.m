function [patchHandles] = boxHistComb(x,y, binWidth, normwidthBoxPlot, hgapBoxDist, sizeHist )
%boxHistComb
...
yc{length(yc)+1}= NaN(size(yc{length(yc)})); % NEW MOD
% creating extra group for distributions
xc= [x,x(end)+mean(diff(x))]; % NEW MOD
...
for i = 1:size(y,2)
    ...
    xm = [zeros(1,numel(height)); repelem(height,2,1); zeros(2,numel(height))] + xCoordinate(end)+ hgapBoxDist ; % MOD
...
pdfxShift = pdfxNorm - min(pdfxNorm,[],1) + xCoordinate(end)+ hgapBoxDist; %MOD
...
xlim([min(xCoordinate)-0.5*mean(diff(xCoordinate)),( xCoordinate(end)+hgapBoxDist+maxHeight*sizeHist*1.1)]) %MOD
