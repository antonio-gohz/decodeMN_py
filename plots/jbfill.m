function[fillhandle,msg]=jbfill(xpoints,upper,lower,color,edge,add,transparency,visibility)
...
if length(upper)==length(lower) && length(lower)==length(xpoints)
    ...
    set(fillhandle,'EdgeColor',edge,'FaceAlpha',transparency,'EdgeAlpha',transparency,'HandleVisibility',visibility);%set edge color
    if add
        hold off
    end
else
