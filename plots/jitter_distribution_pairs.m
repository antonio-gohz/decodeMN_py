...
%      'nGroup' groups n times (e.g. n=2 --> pairs) for Internla PlotType
%
...
for j = 1:length(varargin)
   ...
   if isequal(varargin{j},'Markersize')
    ...
    if isequal(varargin{j},'ScaleDist')
      scale_dist = varargin{j+1}; 
   end
   ...
   if isequal(varargin{j},'nGroup')
       n_group = varargin{j+1};
   end
   if isequal(varargin{j},'subplots')% hardcoded for 2 subplots external only
       n_subplots = varargin{j+1};
   end
end
...
switch plot_type 
    case 'External'
        % Jitter plot 
        if isempty(n_subplots) % hardcoded for 2 subplots external only
            Fig.jit = subplot(6,4,[1 2 3 5 6 7 9 10 11 13 14 15 17 18 19 21 22 23]);
        else 
            Fig.jit = subplot(12,4,(n_subplots-1)*24+[1 2 3 5 6 7 9 10 11 13 14 15 17 18 19 21 22 23]);
        end
        hold on 
        ...
        set(Fig.jit, 'XTick', (1:length(catnames))+0.1, 'XTickLabels', catnames);
        ...
        if isempty(n_subplots) % hardcoded for 2 subplots external only
            ...
            Fig.dist = subplot(12,4,(n_subplots-1)*24+[4 8 12 16 20 24]);
        end
        linkaxes([Fig.jit,Fig.dist],'y')
        axis tight
        hold on 
    ...
    case 'Internal'
        ...
        for n = 1:length(catnames)
            ...
            switch dist_type
                case 'Kernel'
                    scale = scale_dist/max(ydens); 
                case 'Gaussian'
                    scale = scale_dist/max(ydens); 
            end 

            Jitter_distribution(thisData, n, cell2mat(cols(n)), marker_size, line_width, cap_size, dist_type, scale, n_group);
...
function Jitter(data, pos, col, marker_size, line_width, cap_size)
    ...
    value(1) = min(data);
    value(end) = max(data);
    try  % quick Fix for pertert only one recording in subject 2 so it does not work 
    ...
    boxplot(data,'PlotStyle','compact', 'Positions',pos+0.20,'Color',col,'Notch','on');
...
function Jitter_distribution(data, pos, col, marker_size, line_width, cap_size, plot_type, scale, n_group)
    [density, value] = ksdensity(data);          
    density = density(value >= min(data) & value <= max(data));
    value = value(value >= min(data) & value <= max(data));
    value(1) = min(data);
    ...
    offset1 = pos + ceil(pos/n_group)-1; % offset for skiping one space after each pair, this is for the distribution

    scatter(offset1 + jit.*jitterstrength, data, marker_size, 'filled','MarkerEdgeColor','None','MarkerFaceColor',col,'MarkerFaceAlpha',0.6);
    ...
    switch plot_type 
        case 'Gaussian'
            ...
            y_norm(end) = 0; 
            y_norm = y_norm*scale; 
            offset = ceil(pos/n_group)*n_group+1 -0.3; % this makes the offset the same for every pair 
            plot(y_norm+offset,xnormdis,'Color',col,'LineWidth',0.7*line_width)
            patch(y_norm+offset,xnormdis,col,'FaceAlpha',0.4,'EdgeColor','none')
        case 'Kernel'
            [f, xi] = ksdensity(data); 
            f = f*scale; 
            min_f = min(f); 
            offset = ceil(pos/n_group)*n_group+ceil(pos/n_group) -0.3; % this makes the offset the same for every pair 
            plot(f+offset,xi,'Color',col,'LineWidth',0.7*line_width,'HandleVisibility','off')
            patch(f+offset,xi,col,'FaceAlpha',0.4,'EdgeColor','none','HandleVisibility','off')
    end 
   
    boxplot(data,'PlotStyle','compact', 'Positions',offset1+0.20,'Color',col,'Notch','on');
%     errorbar(offset1+0.20,mean(data),std(data),'Color','k','LineWidth',line_width,'CapSize',cap_size,'HandleVisibility','off');
%     scatter(offset1+0.20, mean(data),marker_size,'filled','MarkerFaceColor','k','MarkerEdgeColor', 'None','HandleVisibility','off');
