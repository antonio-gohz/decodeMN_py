function [neuralActivation, normSat] = activationDynamics(excitation, dischargeRate, t, Ap, tc, thr, twitchModel, satLevel, tetParam, tetanusModel, c1,c2, shapeFactor_ ,activationScale_, force,...
    ...
    T = t(2)-t(1);
    normSat = 1/size(excitation,2).* satLevel;
   ...
   % thr = tc.*2.3;
    ...
    if normalization==1 % rec curve witwith generic curve  
                updatedTwitchFactor = zeros([1,size(excitation,2)]);
%                 figure;
        r=[0,0.4,0.75,1];
...
%         end
    ...
    i=1;
    %%
    for n = 3:length(excitation)
        
        if sum(excitation(n,:))>0
            n_spike=n_spike+1;
            t0 = t(1)+(n-1)*T; % as n starts with 1
            firstSpike=1;
            t0s(n_spike,excitation(n,:)>0)=t0;
        end
        ...
        if i<size(tStamps,1) && n>floor(fs*tStamps(i+1,1)) 
            i=i+1; 
        end
            ...
            elseif strcmp(twitchModel,'Fuglevand')
                if normalization==0   % test og normalization
                    neuralActivation(n,:) = (2*exp(-T./tc))'.*neuralActivation(n-1,:)  -  (exp((-2*T)./tc))'.*neuralActivation(n-2,:)  ...
                        + (((Ap*T)./tc).*exp(1-T./(tc)))'.*excitation(n-1,:);
                elseif normalization ==1
                    % updateTwitchFactor updates the twitch only when a new
                    % a new twitch appears 
                    updatedTwitchFactor(logical(excitation(n-1,:))) = 1.5*recCurve(abs(force(n)))+0.5;
                    neuralActivation(n,:) = (2*exp(-T./tc))'.*neuralActivation(n-1,:)  -  (exp((-2*T)./tc))'.*neuralActivation(n-2,:)  ...
                        + ((((normFactor(i))*Ap*T)./tc).*exp(1-T./(tc)))'.*updatedTwitchFactor.*excitation(n-1,:);
...
%                         + ((((normFactor(i)*recCurve(force(n)))*Ap*T)./tc).*exp(1-T./(tc)))'.*excitation(n-1,:);
                 elseif normalization ==2
                    ...
                    updatedTwitchFactor(logical(excitation(n-1,:))) = ((log(100*abs(force(n-1)))/m) - 2.1178)./100;
                    if  updatedTwitchFactor(logical(excitation(n-1,:)))>=1 %saturating to 1 
                        updatedTwitchFactor(logical(excitation(n-1,:)))=1;
                    end
...
% end
                ...
                elseif normalization >=3
                    neuralActivation(n,:) = (2*exp(-T./tc))'.*neuralActivation(n-1,:)  -  (exp((-2*T)./tc))'.*neuralActivation(n-2,:)  ...
                        + (((((normFactor(i)))*abs(force(n)-force(n-1))*Ap*T)./tc).*exp(1-T./(tc)))'.*excitation(n-1,:);
                end
            elseif strcmp(twitchModel,'CEINMS')
                neuralActivation_(n,:) =  alpha_*excitation(n,:) - beta1_*neuralActivation_(n-1,:)  - ...
